...
describe("Directory FS API route POST route", () => {
  ...
  it("can create directory on subdir", async () => {
    ...
    expect(
      await fileVerifyFS(filesystemstorageroot, "test-2", "test-3")
    ).toEqual(true);
  });
  ...
  it("can create file on sub dir", async () => {
    ...
    expect(
      await fileVerifyFS(filesystemstorageroot, "test-4", "fire.pdf")
    ).toEqual(true);
  });
  ...
  it("can create file with different name", async () => {
    ...
    expect(
      await fileVerifyFS(filesystemstorageroot, "another-fire.jpg")
    ).toEqual(true);
  });
  ...
  it("cannot create file when schema is missing and required", async () => {
    ...
    try {
      ...
      expectStatus(res, 400);
      expect(await fileVerifyFS(filesystemstorageroot, "fire.pdf")).toEqual(
        false
      );
    } finally {
