...
export const truncateS3 = async () => {
  const data = await getS3Storage().s3.listObjects({ Bucket: config.get("storage").s3.bucket }).promise()
  if (data.Contents?.length) {
    const delParams: DeleteObjectsRequest = {
      Bucket: config.get("storage").s3.bucket,
      Delete: {
        Objects: data.Contents.map(obj => ({ Key: obj.Key } as ObjectIdentifier)),
      },
    };
...
export const deleteS3Bucket = async () =>
  await getS3Storage().s3.deleteBucket({ Bucket: config.get("storage").s3.bucket }).promise();
...
export const expectStatus200 = (response: supertest.Response) => expectStatus(response, 200);

export const listRequest = (path: string) => serverRequest
  .get(path)
  .set("Authorization", `Bearer ${allAccessToken}`)
  .then(res => res);

export const getRequest = (path: string) => serverRequest
  .get(path)
  .set("Authorization", `Bearer ${allAccessToken}`)
  .then(res => res);

export const postDirectoryRequest = (path: string, name: string, schema: string = "", metadata: any = {}) =>
  serverRequest
  .post(path)
  .set('Authorization', `Bearer ${allAccessToken}`)
...
export const putDirectoryRequest = (path: string, schema: string = "", metadata: any = {}) =>
  serverRequest
  .put(path)
  .set('Authorization', `Bearer ${allAccessToken}`)
  .send({
...
export const postFileRequest = (filepath: string, file: string, schema: string = "", metadata: any = {}, filename: string = "") =>
  serverRequest
  .post(filepath)
  .set("Authorization", `Bearer ${allAccessToken}`)
  .field("type", "file")
...
export const putFileMetaRequest = (filepath: string, schema: string = "", metadata: any = {}) =>
  serverRequest
  .put(filepath)
  .set("Authorization", `Bearer ${allAccessToken}`)
  .send({
...
export const fileVerifyS3 = async (...filePieces: string[]) => {
  ...
  const data = await getS3Storage().s3.listObjects({ Bucket: config.get("storage").s3.bucket, Prefix: isFile ? `${path.join(...filePieces)}` : `${path.join(...filePieces)}/` }).promise();
